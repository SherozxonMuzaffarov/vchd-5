            ...
            <div class="card shadow-sm" style="overflow: auto; white-space: nowrap">
                ...
                <BTableSimple striped="true" hover="true" bordered="true" class="mt-4">
                    <BThead >
                        <BTr >
                            <BTd >Дата</BTd>
                            <BTd >Колечество <br> испитанных <br> наружных <br> пружин</BTd>
                            <BTd >Забракована</BTd>
                            <BTd >Колечество <br> испитанных <br> внутренных <br> пружин</BTd>
                            <BTd >Забракована</BTd>
                            <BTd >Фамилия и <br> подпись слесаря <br> производившего <br> испитание</BTd>
                            <BTd >Фамилия и <br> подпись <br> бригадира</BTd>
                        </BTr>
                    </BThead>
                    <BTbody>
                        <BTr v-show="addToggle"> 
                            <BTd><BFormInput v-model="formData.register_time" /></BTd>
                            <BTd><BFormInput v-model="formData.external_quantity" /></BTd>
                            <BTd><BFormInput v-model="formData.external_rejected" /></BTd>
                            <BTd><BFormInput v-model="formData.internal_quantity" /></BTd>
                            <BTd><BFormInput v-model="formData.internal_rejected" /></BTd>
                            <BTd>
                            ...
                            <BTd>
                                <BFormSelect v-model="formData.brigader" :options="employeeList" >
                                    <template #first>
                                    <BFormSelectOption :value="null" disabled>--бригадира --</BFormSelectOption>
                                    </template>
                                </BFormSelect>
                            </BTd>
                        ...
                        <BTr class="align-middle" v-for="item in Data" :key="item._id">
                            ...
                            <BTd >{{ item?.external_quantity }}</BTd>
                            <BTd>{{ item?.external_rejected }}</BTd>
                            <BTd>{{ item?.internal_quantity }}</BTd>
                            <BTd>{{ item?.internal_rejected}}</BTd>
...
const saveData = async() => {
    try {
        if (formData.value._id) {
            ...
            if(isConfirmed) {
                let res = await axios.patch("/api/telejka-sexi/prujina/update/" + formData.value._id, formData.value);
                if (res.data) {
                    addToggle.value = !addToggle.value
                    makeFormNull();
                    getAll();
            ...
            if(isConfirmed) {
                let res = await axios.post("/api/telejka-sexi/prujina/create", formData.value);
                if (res.data) {
                    addToggle.value = !addToggle.value
                    makeFormNull();
                    getAll();
...
const makeFormNull = () => {
    ...
    formData.value.external_quantity= null,
    formData.value.external_rejected= null,
    formData.value.internal_quantity= null,
    formData.value.internal_rejected= null,
...
const getAll = async() => {
    try {
        let res = await axios.get(`/api/telejka-sexi/prujina/all?status=${encodeURIComponent(groupedSelected.value)}`);
        if (res.data) { 
            Data.value = res.data;
        }
...
const getOne = async(id, status) => {
    try {
        let res = await axios.get("/api/telejka-sexi/prujina/one/" + id  + '/' + status);
        if (res.data) {
            addData()
