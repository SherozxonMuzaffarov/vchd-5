...
const handleOk = async () => {
  try {
    let res = await axios.post("/api/owner/create", FormData.value);
    if (res) {
      getAll();
...
const handleUpdate = async () => {
  ...
  try {
    let res = await axios.patch("/api/owner/update/" + id, FormData.value);
    if (res) {
      getAll();
...
const deleteItem = async (id) => {
  try {
    ...
    if (confirmDelete) {
      try {
        let res = await axios.delete("/api/owner/delete/" + id);
...
let getOne = async (id) => {
  try {
    let res = await axios.get("/api/owner/one/" + id);
    if (res.data) {
